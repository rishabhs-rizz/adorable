...
// Controller Fn to Make Revision (revise our code by adding a new prompt)
export const makeRevision = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // updated prompt/message added to the cnversation
    ...
    //update user credits by decrementing 5
    await prisma.user.update({
      where: { id: userId },
      data: { credits: { decrement: 5 } },
    });

    //enhance the project by sending the conversation to openai and getting the revised code
...
Return ONLY the enhanced request, nothing else. Keep it concise (1-2 sentences).`,
    ...
    Apply the requested changes while maintaining the Tailwind CSS styling approach.`,
        ...
        {
          role: "user",
          content: `here's the current website code: "${currentProject.current_code}" & The User wants this change: "${enhancedPrompt}"`,
        },
      ],
    });
    ...
    res.json({
      message: "Revision changes made successfully",
    });
  } catch (error: any) {
    await prisma.user.update({
      where: { id: userId },
...
// Controller Fn to rollback to a previous version
export const rollbackToVersion = async (req: Request, res: Response) => {
  try {
    ...
    await prisma.conversation.create({
      data: {
        role: "assistant",
        content: `You've rolled back to a previous version. You can now preview the website.`,
        projectId,
      },
    });
