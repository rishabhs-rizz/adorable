...
// Controller Fn to create a new Project
export const createUserProjects = async (req: Request, res: Response) => {
  ...
  try {
    ...
    //Second Conversation entry after enhanced prompt
    await prisma.conversation.create({
      data: {
        role: "assistant",
    ...
    // Save the generated code as a new message in the conversation
    await prisma.conversation.create({
      data: {
        role: "assistant",
        content:
          "I've generated your website! You can preview it and if you want any changes or improvements, just let me know!",
        projectId: project.id,
      },
    });
    ...
    await prisma.user.update({
      where: { id: userId },
      data: { credits: { increment: 5 } }, //if any error occurs, refund the user with 5 credits
    });
    console.error(error.code || error.message);
    res.status(500).json({ error: "Internal Server Error" });
  }
